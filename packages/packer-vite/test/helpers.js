...
/**
 * The factory resolves paths against INIT_CWD, which yarn sets but `node --test` does not. Vite
 * silently falls back to process.cwd() and picks up this package's own tsconfig.json, so every test must
 * pin it explicitly rather than rely on a default.
 */
...
function viteConfig(opts = {}, { app = 'ts-app' } = {}) {
    return withInitCwd(app, () => packer.createApplicationConfiguration(opts));
}
